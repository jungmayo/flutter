...
//       }
//
//       // 1. JWT 토큰을 안전한 금고에 보관 // I/O (데이터 입출력)
//       // I/O는 시간이 많이 걸리기 때문에 비동기 처리
...
//           isLogin: true);
//
//       // 3. 로그인 이후 인증권한 필요한 API 접근시에
//       // 매번 금고에 접근해서 dio 속성 값 변경해서 호출하는 것은 불편함
//       // dio 객체를 전역변수로 설계
//       // dio 객체에 접근해서 속성값을 바꿀 수 있다는 뜻
//       // 액세스 토큰 설정
//       // Dio 헤더에 JWT 토큰 설정(객체 상태값 추가)
//       dio.options.headers['Authorization'] = accessToken;
//
//       // 화면 이동 처리
//       // Navigator.pushNamed(mContext, '/post/list');
//       // 근데 pushedNamed라고 계속 호출을 하면 Navigator stack 메모리에 계속 쌓임..
//       // 뒤로가기 하면 쌓인만큼 계속 뒤로갈수있음
//       // 이전에 쌓여있던 stack(화면) 다 파괴하면서 이동시킬것
//       Navigator.popAndPushNamed(mContext, '/post/list');
//       // 모든 예외처리가 설정 된다.
//       // 특정 예외만 잡고싶으면 on DioException
//     } catch (e, stackTrace) {
//       // 1. Ip 주소가 잘못 되었을 때
//       // 2. 서버가 종료 되었을 때
//       // 3. 트래픽이 몰려서 서버 연결 시간 초과
...
//       // ExceptionHandler.handleException('회원ㄱ입성공', StackTrace.current);
//       // 바로 화면 이동 처리
...
//
// // 창고 관리자 선언 ( 창고 = 뷰 모델)
// //<>에는 관리하고 싶은 창고를 넣기 , 관리하고자 하는 타입 넣기
// // 어떤 창고를 관리할지 지정해야 함
